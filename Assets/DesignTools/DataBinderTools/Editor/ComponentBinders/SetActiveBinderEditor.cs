...
[CustomEditor(typeof(SetActiveBinderComponent))]
public class SetActiveBinderEditor : Editor
{
    ...
    {
        m_bindersProperty = serializedObject.FindProperty("m_gameObjectEnablerBinders");
    }
    ...
    public override void OnInspectorGUI()
    {
        ...
        if (GUILayout.Button("Add GameObject Enabler Binder", GUILayout.Width(200)))
        {
    ...
    private void DisplayDataBinders()
    {
        for (int i = 0; i < m_bindersProperty.arraySize; i++)
        {
            ...
            m_bindersProperty.GetArrayElementAtIndex(i).isExpanded = EditorGUILayout.Foldout(m_bindersProperty.GetArrayElementAtIndex(i).isExpanded, "GameObject Enabler Binder " + (i + 1));
    ...
    private void DisplayFoldout(int index)
    {
        ...
        for (int i = 0; i < targetsProperty.arraySize; i++)
        {
            ...
            targetsProperty.GetArrayElementAtIndex(i).objectReferenceValue = (GameObject)EditorGUILayout.ObjectField(targetsProperty.GetArrayElementAtIndex(i).objectReferenceValue, typeof(GameObject), true);
            EditorGUILayout.EndHorizontal();
        }
        GUILayout.Space(5);
        EditorGUILayout.EndVertical();
